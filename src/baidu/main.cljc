(ns baidu.main
  "Kotodama WASM entrypoint for the Baidu clean-room actor (L4) — Clojure port.
  ...
  facts (`baidu.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Order"     :plural "orders"     :id-prefix "baidu_ord"
   ...
   {:entity "Tracking"  :plural "trackings"  :id-prefix "baidu_tra"
...
(defn emit-facts
  "EAVT facts for one record: {\"baidu.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "baidu-compat" :tier tier :entities entities} 200])
